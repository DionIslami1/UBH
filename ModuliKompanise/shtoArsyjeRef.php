...
{	
	...
	if(empty($Kerkesa)) {	
		...
		if(empty($Kerkesa)) {
			echo "<font color='red'>Kerkesa field is empty.</font><br/>";
		}			
	} else {	
		//updating the table
		mysqli_query ($conn_hd,"SET @id = ${ID_Kerkesa}");
		mysqli_query ($conn_hd,"SET @ker = '${Kerkesa}'");
		$rezultat = mysqli_query($conn_hd,"CALL modifikoArsyje(@id,@ker)");
		if ($rezultat) {
			# code...
		...
		<div id="page-wrapper">
			...
			<!-- Banner -->
				<section style="padding-top: 0%;" id="features">
					<div class="container">
						<header>
						<h2>Shkruaj aprovimin apo refuzimin e kerkeses:</h2>
						</header>
						<form method="post" action="shtoArsyjeRef.php">
							<div class="row">
							
							<div class="col-6 col-12-medium" style="margin-left: 22%; width: 50%">
							<p style="margin: 0%">Kerkesa: </p>
							<textarea name="Kerkesa" id="demo-name"></textarea>
							</div>
							
							<div class="col-12">
							<input type="hidden" name="ID_Kerkesa" value='<?php echo $_GET['ID_Kerkesa'];?>' />
							<td><input type="submit" name="shtoArsyjeRef"  value="Dergo"></td>
