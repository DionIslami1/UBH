...
if(isset($_POST['shtoDatenKthimit']))
{	
	$ID_Kerkesa = $_POST['ID_Kerkesa'];
	$Data_Kthimit_Huazimit=$_POST['Data_Kthimit_Huazimit'];
	// checking empty fields
	if(empty($Data_Kthimit_Huazimit)) {	

		if(empty($Data_Kthimit_Huazimit)) {
			echo "<font color='red'>Data_Kthimit_Huazimit field is empty.</font><br/>";
		}			
	} else {	
		//updating the table
		mysqli_query ($conn_hd,"SET @id = ${ID_Kerkesa}");
		mysqli_query ($conn_hd,"SET @DataKthimit = '${Data_Kthimit_Huazimit}'");
		$rezultat = mysqli_query($conn_hd,"CALL modifikoDataKthimitHuazimit(@id,@DataKthimit)");
		if ($rezultat) {
			# code...
		...
		<div id="page-wrapper">
			...
			<!-- Banner -->	
				<section style="padding-top: 0%;" id="features">
					<div class="container">
						<header>
						<h2>Shkruaj Daten e Kthimit </h2>
						</header>
						<form method="post" action="shtoDatenKthimit.php">
							<div class="row">
							<div class="col-6 col-12-medium" style="margin-left: 20%; width: 50%">
							<p style="margin: 0%">Data e Kthimit</p>
							<input style="color: black;" type="date" name="Data_Kthimit_Huazimit" id="Data_Kthimit_Huazimit" value="" placeholder="Data_Kthimit_Huazimit" />
							...
							<td><input type="submit" name="shtoDatenKthimit" value="Shto"></td>
							</div>
							</div>
						</form>
						</div>
