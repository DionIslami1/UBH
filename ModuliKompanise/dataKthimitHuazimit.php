...
if(isset($_POST['dataKthimitHuazimit']))
{	
	$ID_Kerkesa = $_POST['ID_Kerkesa'];
	$Data_Kthimit=$_POST['Data_Kthimit'];
	// checking empty fields
	if(empty($Data_Kthimit)) {	

		if(empty($Data_Kthimit)) {
			echo "<font color='red'>Data_Kthimit field is empty.</font><br/>";
		}			
	} else {	
		//updating the table
		mysqli_query ($conn_hd,"SET @id = ${ID_Kerkesa}");
		mysqli_query ($conn_hd,"SET @DataKthimit = '${Data_Kthimit}'");
			...
			<!-- Banner -->	
				<section style="padding-top: 0%;" id="features">
					<div class="container">
						...
						<form method="post" action="dataKthimitHuazimit.php">
							<div class="row">
							<div class="col-6 col-12-medium" style="margin-left: 20%; width: 50%">
							<p style="margin: 0%">Data e Kthimit</p>
							<input style="color: black;" type="date" name="Data_Kthimit" id="Data_Kthimit" value="" placeholder="Data Kthimit" />
							...
							<td><input type="submit" name="dataKthimitHuazimit" value="Shto"></td>
							</div>
							</div>
						</form>
						</div>
