...
{	
	...
	if(empty($Kthimi)) {	
		...
		if(empty($Kthimi)) {
			echo "<font color='red'>Kthimi field is empty.</font><br/>";
		}			
	} else {	
		//updating the table
		mysqli_query ($conn_hd,"SET @id = ${ID_Kerkesa}");
		mysqli_query ($conn_hd,"SET @Kthimi = '${Kthimi}'");
		$rezultat = mysqli_query($conn_hd,"CALL modifikoKthimin(@id,@Kthimi)");
		if ($rezultat) {
			# code...
		...
		<div id="page-wrapper">
			...
			<!-- Banner -->
				<section style="padding-top: 0%;" id="features">
					<div class="container">
						<header>
						<h2>Shkruaj a eshte bere kthimi </h2>
						</header>
						<form method="post" action="shtoKthim.php">
							<div class="row">													
							<div class="col-6 col-12-medium" style="margin-left: 20%; width: 50%">
							<p style="margin: 0%">Kthimi:</p>
							<input style="color: black;" type="text" name="Kthimi" id="Kthimi" value="" placeholder="Kthimi" />
							...
							<td><input type="submit" name="shtoKthim" value="Dergo"></td>
							</div>
							</div>
						</form>
						</div>
